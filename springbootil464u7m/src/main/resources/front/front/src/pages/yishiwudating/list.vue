...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px 0","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","display":"flex","width":"100%","borderStyle":"solid","height":"auto","order":"2"}'>
      <el-form-item :style='{"margin":"0 5px 0px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>物品名称：</div>
        <el-input v-model="formSearch.wupinmingcheng" placeholder="物品名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  <el-button v-if=" true " :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0px 10px 0 0px","color":"#fff","display":"inline-block","outline":"none","borderRadius":"30px","background":"#f5bb00","width":"90px","fontSize":"14px","lineHeight":"40px","height":"40px"}' type="primary" @click="getList(1, curFenlei)"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"16px"}' class="el-icon-search"></i>查询</el-button>
	  <el-button v-if="btnAuth('yishiwudating','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 15px","margin":"0px 0px 0 0","color":"#fff","display":"inline-block","outline":"none","borderRadius":"30px","background":"#1c6a6d","width":"90px","fontSize":"14px","lineHeight":"40px","height":"40px"}' type="primary" @click="add('/index/yishiwudatingAdd')"><i v-if="false" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"minHeight":"699px","padding":"20px 0","margin":"20px 0 0","borderRadius":"0px","background":"none","flex":"1","width":"calc(100% - 310px)","float":"left","order":"5"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0 0 0 2%","color":"#999","flexWrap":"wrap","background":"none","display":"flex","width":"100%","fontSize":"14px","height":"auto"}'>
			<div :style='{"border":"0px solid #ddd","cursor":"pointer","padding":"0 0 0px","boxShadow":"1px 2px 9px #ccc","margin":"0 2% 30px 0","display":"flex","justifyContent":"flex-end","overflow":"hidden","flexWrap":"wrap","background":"#fff","width":"23%","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<img @click.stop="imgPreView(item.mohutupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"240px"}' v-if="item.mohutupian && item.mohutupian.substr(0,4)=='http'" :src="item.mohutupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.mohutupian?item.mohutupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"240px"}' v-else :src="baseUrl + (item.mohutupian?item.mohutupian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"0 10px","borderColor":"#ddd","margin":"0 0 6px","whiteSpace":"nowrap","color":"#333","textAlign":"left","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"32px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"dashed"}' class="name ">{{item.wupinmingcheng}}</div>
				<div :style='{"padding":"0 10px","borderColor":"#ddd","margin":"0 0 6px","whiteSpace":"nowrap","color":"#333","textAlign":"left","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"32px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"dashed"}' class="name ">{{item.wupinleixing}}</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
		this.$http.get('wupinleixing/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'wupinleixing'
      },
      getList(page, fenlei, ref = '') {
        ...
        if (this.curFenlei != '全部') searchWhere.wupinleixing = this.curFenlei;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`yishiwudating/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/yishiwudatingDetail', query: params});
      },
